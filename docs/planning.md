...
| 编号 | 类别 | 内容 |
|------|------|------|
| P1 | 性能 | 并发页面渲染 — goroutine pool，100 篇 ~500ms |
| P2 | 性能 | 模板预编译缓存 — 源文件缓存 + Engine 跨构建复用 |
| E1 | 阅读 | 字体大小调节 — 小/中/大三级，localStorage 记忆 |
| E2 | 阅读 | 阅读宽度切换 — 窄栏/宽栏 |
| E3 | 阅读 | 暗色模式亮度微调 — 柔和/适中/深邃 |
| E4 | 首页 | 文章卡片封面图 — front matter `cover` 字段 |
| E5 | 首页 | Tag 快速筛选 — 首页 tag 横条前端过滤（已移除） |
| E6 | 首页 | 置顶文章 — front matter `pinned: true` |
| E7 | 打印 | 打印样式优化 — 隐藏导航/TOC/侧栏，链接展开脚注 |
| E8 | 快捷键 | 键盘导航 — j/k/?/gh/gt |
| E9 | 代码 | 行号/换行切换（已移除） |
| E10 | 标签页 | 标签云视觉增强 — 水墨风格卡片，字号双维度区分权重 |
| E11 | 标签页 | 标签描述 — config.yaml `tagDescriptions` 映射 |
| E12 | 标签页 | 文章数 + 最近更新 — "共 N 篇，最近更新：YYYY-MM" |
| E13 | 归档页 | 日历热力图（已移除） |
...
| 编号 | 问题 | 优先级 |
|------|------|--------|
| R1 | RSS 阅读体验不佳——夫君实测无法流畅阅读（2026-08-15 标记，现象与原因待查，暂缓处理） | 待查 |
