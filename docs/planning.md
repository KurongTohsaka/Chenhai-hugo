...
| 任务 | 说明 |
|------|------|
| 日常 bug 修复 | 日常使用中收集的小问题 |

### v0.7.0 — 图片体系 + 构建性能 + 工具链

| 编号 | 类别 | 任务 | 说明 |
|------|------|------|------|
| F1 | 图片 | WebP 自动转换 | 构建时 jpg/png → webp，体积缩小 60-80%，保留原图兼容 |
| F2 | 图片 | 响应式 srcset | 生成 480w/800w/1200w 多尺寸变体 |
| F3 | 图片 | chenhai image | 子命令：compress（压缩）、convert（格式转换）、resize（尺寸调整） |
| F4 | 图片 | 图片懒加载完善 | 首屏 eager、非首屏 lazy、占位色 |
| F5 | 性能 | 并发页面渲染 | goroutine pool，100 篇从 ~2s → ~0.5s |
| F6 | 性能 | 模板预编译缓存 | 首次解析后缓存，增量构建跳过模板解析 |
| F7 | 性能 | 构建耗时面板 | 各阶段耗时表格 + 缓存命中率 |
| F8 | 工具 | chenhai doctor | 检测 config.yaml 语法、content/ 结构完整性、静态资源有效性 |
| F9 | 写作 | 相关文章优化 | 分类加权 + 时效衰减 |
