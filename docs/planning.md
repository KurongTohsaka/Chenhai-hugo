...
| 任务 | 说明 |
|------|------|
| 标题锚点链接 | hover 标题显示 # 图标，点击复制含 hash 的 URL |
| 复制代码去行号 | 复制按钮过滤代码块左侧行号列内容 |
...
| Series 系列文章 | front matter `series` 字段，文章间上下篇导航 |
| 面包屑导航 | 分类/标签页顶部路径 `首页 > 分类 > Go` |
| 构建耗时统计 | `chenhai build` 各阶段耗时输出 |
| Draft 预览模式 | `chenhai serve --drafts` 本地预览草稿 |
