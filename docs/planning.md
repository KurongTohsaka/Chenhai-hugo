...
### v0.7.0 — 构建性能 + 阅读体验 + 工具链

| 编号 | 类别 | 任务 | 说明 |
|------|------|------|------|
| P1 | 性能 | 并发页面渲染 | goroutine pool，100 篇从 ~2s → ~0.5s |
| P2 | 性能 | 模板预编译缓存 | 首次解析后缓存，增量构建跳过模板解析 |
| E1 | 阅读 | 字体大小调节 | 小/中/大三级切换，localStorage 记忆 |
| E2 | 阅读 | 阅读宽度切换 | 窄栏（680px）/ 宽栏切换 |
| E3 | 阅读 | 暗色模式亮度微调 | 深/中/浅三种暗度 |
| E4 | 首页 | 文章卡片封面图 | front matter `cover` 字段，卡片缩略图 |
| E5 | 首页 | Tag 快速筛选 | 首页顶部 tag 横条，前端过滤不跳页 |
| E6 | 首页 | 置顶文章 | front matter `pinned: true` 永远排最前 |
| E7 | 打印 | 打印样式优化 | 隐藏导航/TOC/侧栏，链接展开脚注 |
| E8 | 快捷键 | 键盘导航 | j/k 上下文章、? 快捷键面板 |
| E9 | 代码 | 行号/换行切换 | 按钮切换自动换行 |
| E10 | 标签页 | 标签云视觉增强 | 渐变/彩色标签卡片，字号+颜色双维度区分权重 |
| E11 | 标签页 | 标签描述 | config.yaml 中为标签配置描述文字，标签页顶部展示 |
| E12 | 标签/分类 | 文章数 + 最近更新 | 每个分类/标签下显示"共 N 篇，最近更新：2026-06" |
| E13 | 归档页 | 日历热力图 | GitHub 风格热力图，直观展示写作频率 |
| E14 | 标签页 | 排序切换 | 标签云支持按名称/文章数/最近更新排序 |
| T1 | 工具 | chenhai doctor | 检测 config.yaml 语法、content/ 结构完整性 |
| T2 | 写作 | 相关文章优化 | 分类加权 + 时效衰减 |
